...
if (loginStart > -1 && registerEnd > -1) {
  const newContent = `    <!-- Login Page (Hidden initially) -->
    <div id="view-login" class="hidden w-full min-h-screen bg-white dark:bg-[#070B19]">
      <div class="flex min-h-screen flex-row">
        ...
        <div class="hidden lg:flex flex-col justify-between w-1/2 relative overflow-hidden bg-gradient-to-br from-slate-900 to-[#0A0F1F] p-12 lg:px-16 lg:py-12">
          <!-- Ambient Effects -->
          ...
          <div class="relative z-10 max-w-lg mb-20">
            <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-blue-400 text-sm font-bold tracking-widest uppercase mb-8 backdrop-blur-sm">
              <span class="w-2 h-2 rounded-full bg-blue-400 animate-pulse"></span>
              Selamat Datang Kembali
            </div>
            <h1 class="text-4xl lg:text-5xl font-extrabold text-white mb-6 leading-[1.2] tracking-tight">Lanjutkan <span class="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400">perjalanan karir</span> Anda.</h1>
            <p class="text-slate-400 text-lg leading-relaxed">Masuk ke jagoCV untuk mengelola portofolio, mensunting CV, dan menemukan peluang baru. Kami siap membantu Anda menembus karir impian.</p>
          </div>
        ...
        <div class="flex-1 flex items-center justify-center p-6 sm:p-12 w-full lg:w-1/2">
          <div class="w-full max-w-md mx-auto animate-[slideUp_0.5s_ease_forwards]">
            <!-- Mobile Brand -->
            <div class="flex lg:hidden items-center justify-start gap-3 mb-10 cursor-pointer" onclick="document.getElementById('view-login').classList.remove('flex'); document.getElementById('view-login').classList.add('hidden'); document.getElementById('view-landing').classList.remove('hidden'); document.getElementById('view-landing').classList.add('flex', 'flex-col');">
            ...
            <div class="space-y-6">
              ...
              <form class="space-y-4 text-left" onsubmit="event.preventDefault();">
                <div>
                  ...
                  <input type="email" class="w-full bg-white dark:bg-[#0B1221] border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3.5 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all shadow-sm" placeholder="nama@email.com">
                </div>
                <div>
                  ...
                  <input type="password" class="w-full bg-white dark:bg-[#0B1221] border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3.5 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all shadow-sm" placeholder="••••••••">
                </div>
                
                <div class="pt-2">
                  <button type="submit" id="btn-login-submit" class="w-full flex items-center justify-center gap-2 bg-slate-900 hover:bg-black dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100 text-white font-bold py-3.5 px-4 rounded-xl transition-all shadow-lg shadow-slate-900/10 dark:shadow-none hover:shadow-xl hover:-translate-y-[1px]">
                    Masuk Sekarang
      ...
      <div class="flex min-h-screen flex-row">
        ...
        <div class="hidden lg:flex flex-col justify-between w-1/2 relative overflow-hidden bg-gradient-to-br from-slate-900 to-[#0A0F1F] p-12 lg:px-16 lg:py-12">
          ...
          <div class="relative z-10 max-w-lg mb-10">
            <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-indigo-400 text-sm font-bold tracking-widest uppercase mb-8 backdrop-blur-sm">
              <span class="w-2 h-2 rounded-full bg-indigo-400 animate-pulse"></span>
              Buat Akun Gratis
            </div>
            <h3 class="text-4xl lg:text-5xl font-extrabold text-white mb-10 leading-[1.2] tracking-tight">Satu akun untuk me-roket-kan <span class="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-400">peluang karir Anda.</span></h3>
            
            <div class="grid gap-4">
              <div class="bg-white/5 border border-white/10 rounded-2xl p-5 backdrop-blur-md flex items-center gap-4 transition-transform hover:-translate-y-1 hover:bg-white/10">
                ...
                <div>
                  <h4 class="text-white font-bold text-base mb-0.5">Maksimal Score ATS</h4>
                  <p class="text-slate-400 text-sm leading-relaxed">Template CV yang ramah mesin Applicant Tracking System.</p>
                </div>
              </div>

              <div class="bg-white/5 border border-white/10 rounded-2xl p-5 backdrop-blur-md flex items-center gap-4 transition-transform hover:-translate-y-1 hover:bg-white/10">
                <div class="w-12 h-12 rounded-xl bg-indigo-500/20 text-indigo-400 flex items-center justify-center shrink-0 border border-indigo-500/30">
                  <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"></path></svg>
                </div>
                <div>
                  <h4 class="text-white font-bold text-base mb-0.5">Penulisan Super Cepat & AI</h4>
                  <p class="text-slate-400 text-sm leading-relaxed">Asisten kecerdasan buatan menyusun ringkasan profil hitungan detik.</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="flex-1 flex items-center justify-center p-6 sm:p-12 w-full lg:w-1/2">
          <div class="w-full max-w-md mx-auto animate-[slideUp_0.5s_ease_forwards]">
            ...
            <div class="mb-8 text-left">
              <h2 class="text-3xl font-bold text-slate-900 dark:text-white mb-2">Buat Akun Anda</h2>
              <p class="text-slate-500 dark:text-slate-400">Mulailah membangun masa depan secara gratis.</p>
            </div>
              ...
              <form class="space-y-4 text-left" onsubmit="event.preventDefault();">
                <div class="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" class="w-full bg-white dark:bg-[#0B1221] border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3.5 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all shadow-sm" placeholder="Budi">
                  </div>
                  <div>
                    <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Nama Belakang</label>
                    <input type="text" class="w-full bg-white dark:bg-[#0B1221] border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3.5 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all shadow-sm" placeholder="Setiawan">
                  </div>
                </div>
                <div>
                  <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Email</label>
                  <input type="email" class="w-full bg-white dark:bg-[#0B1221] border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3.5 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all shadow-sm" placeholder="nama@email.com">
                </div>
                <div>
                  <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Password</label>
                  <input type="password" class="w-full bg-white dark:bg-[#0B1221] border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3.5 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all shadow-sm" placeholder="Minimal 8 karakter">
                </div>
                ...
                <p class="text-xs text-center text-slate-500 mt-4 leading-relaxed">
                  Dengan mendaftar, Anda menyetujui <a href="#" class="underline hover:text-slate-800 dark:hover:text-slate-300 transition-colors">Syarat & Ketentuan</a> serta <a href="#" class="underline hover:text-slate-800 dark:hover:text-slate-300 transition-colors">Kebijakan Privasi</a> kami.
                </p>
              </form>
  ...
  console.log("Both auth views fully aligned and updated for max consistency/neatness");
} else {
